...
function Compare-String {
    <#
    .SYNOPSIS
        Compare two strings using the Damerau-Levenshtein distance algorithm.
    .PARAMETER Candidate
        The Candidate string to compare against.
    .PARAMETER Pattern
        The Pattern string to compare with.
    .PARAMETER FuzzyThreshold
        The maximum difference allowed between the two strings.
    .EXAMPLE
        Compare-String -Candidate 'foo' -Pattern 'bar'
    .EXAMPLE
        Compare-String -Candidate 'foo' -Pattern 'bar' -MaxDiff 1
    .OUTPUTS
    FuzzyResults
    .NOTES
    this is just an internal powershell class that i copied and wanted to use as a module.
    .LINK
    https://github.com/PowerShell/PowerShell/blob/master/src/System.Management.Automation/utils/FuzzyMatch.cs
    #>
    [CmdletBinding()]
    [Alias('Get-FuzzyMatch','fuzz')]
    [OutputType([FuzzyResults])]
    param (
        ...
        [int] $FuzzyThreshold = 5
    )
    $Distance = $null
    $FuzzyMatcher = [FuzzyMatcher]::new($FuzzyThreshold)
    $isMatch = $FuzzyMatcher.IsFuzzyMatch($Candidate, $Pattern, [ref]$Distance )
    [FuzzyResults]@{
        Distance       = $Distance
        IsMatch        = $isMatch
        Candidate      = $Candidate
...
if (-Not ('FuzzyMatcher' -as [type])) {
    ...
    public class FuzzyMatcher
    {
        ...
        {
            int unused;
            return IsFuzzyMatch(candidate, pattern, out unused);
        }
        /// <summary>
...
Export-ModuleMember -Function Compare-String -Alias Get-FuzzyMatch,fuzzy
